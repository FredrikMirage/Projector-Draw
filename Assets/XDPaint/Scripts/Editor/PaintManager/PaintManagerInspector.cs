...
namespace XDPaint.Editor
{
    ...
    public class PaintManagerInspector : UnityEditor.Editor
    {
        ...
        private SerializedProperty paintSpaceProperty;
        private SerializedProperty projectionMethodProperty;
        private SerializedProperty projectionDirectionProperty;
        private SerializedProperty projectionMaxAngleProperty;
        private SerializedProperty projectionAngleAttenuationModeProperty;
        private SerializedProperty projectionBrushDepthProperty;
        private SerializedProperty projectionDepthFadeRangeProperty;
        private SerializedProperty projectionEdgeSoftnessProperty;
        private SerializedProperty normalCutoutProperty;
        ...
        private EnumDrawer<PaintSpace> paintSpaceEnumDrawer;
        private EnumDrawer<PaintSpace> paintSpaceDrawer
        {
            get
            {
                if (paintSpaceEnumDrawer == null)
                {
                    paintSpaceEnumDrawer = new EnumDrawer<PaintSpace>();
                    paintSpaceEnumDrawer.Init();
                }
                return paintSpaceEnumDrawer;
            }
        }
        
        private EnumDrawer<ProjectionMethod> projectionMethod;
        private EnumDrawer<ProjectionMethod> projectionMethodDrawer
        {
            get
            {
                if (projectionMethod == null)
                {
                    projectionMethod = new EnumDrawer<ProjectionMethod>();
        ...
        private EnumDrawer<ProjectionDirection> projectionDirection;
        private EnumDrawer<ProjectionDirection> projectionDirectionDrawer
        {
            get
            {
                if (projectionDirection == null)
                {
                    projectionDirection = new EnumDrawer<ProjectionDirection>();
                    projectionDirection.Init();
                }
                return projectionDirection;
            }
        }
        
        private EnumDrawer<AngleAttenuationMode> angleAttenuationMode;
        private EnumDrawer<AngleAttenuationMode> angleAttenuationModeDrawer
        {
            get
            {
                if (angleAttenuationMode == null)
                {
                    angleAttenuationMode = new EnumDrawer<AngleAttenuationMode>();
                    angleAttenuationMode.Init();
                }
                return angleAttenuationMode;
            }
        }
        ...
        void OnEnable()
        {
            ...
            paintSpaceProperty = serializedObject.FindProperty("paintSpace");
            projectionMethodProperty = serializedObject.FindProperty("material.projectionMethod");
            projectionDirectionProperty = serializedObject.FindProperty("material.projectionDirection");
            normalCutoutProperty = serializedObject.FindProperty("material.normalCutout");
            projectionMaxAngleProperty = serializedObject.FindProperty("material.projectionMaxAngle");
            projectionAngleAttenuationModeProperty = serializedObject.FindProperty("material.projectionAngleAttenuationMode");
            projectionBrushDepthProperty = serializedObject.FindProperty("material.projectionBrushDepth");
            projectionDepthFadeRangeProperty = serializedObject.FindProperty("material.projectionDepthFadeRange");
            projectionEdgeSoftnessProperty = serializedObject.FindProperty("material.projectionEdgeSoftness");
        ...
        public override void OnInspectorGUI()
        {
            ...
            if (objectForPaintingProperty.objectReferenceValue != null)
            {
                ...
                {
                    if (PromptToUpdateModelSettings())
                    {
                        UpdateMeshData();
                        MarkAsDirty();
                    }
            ...
            if (objectForPaintingProperty.objectReferenceValue == null)
            {
                ...
                if (paintManager.gameObject.transform.childCount > 0)
                {
                    ...
                    if (compatibleComponents.Count > 0)
                    {
                        ...
                        paintManager.PaintSpace = PaintManagerHelper.IsMeshObject(compatibleComponents[0]) ? PaintSpace.World : PaintSpace.UV;
                        paintSpaceProperty.enumValueIndex = Array.IndexOf(Enum.GetValues(typeof(PaintSpace)), paintManager.PaintSpace);
                        return true;
                    }
                }
        ...
        private void DrawPresetsBlock()
        {
            ...
            EditorGUI.BeginDisabledGroup(PaintController.Instance.UseSharedSettings);
            EditorGUI.BeginChangeCheck();
        ...
        private bool PromptToUpdateModelSettings()
        {
            ...
            if (showHelpBox && sharedMesh != null)
            {
                EditorGUILayout.HelpBox("Enable “Read/Write” on a mesh to access its vertex/index data for painting", MessageType.Warning);
                if (GUILayout.Button(new GUIContent("Set “Read/Write”"), GUILayout.ExpandWidth(true)))
                {
        ...
        private void DrawToolsBlock()
        {
            ...
            var paintSpace = PaintSpace.UV;
            var paintSpaceChanged = paintSpaceDrawer.Draw(paintSpaceProperty, "Paint Space", PaintManagerHelper.PaintingSpaceTooltip, ref paintSpace);
            if (paintSpaceChanged)
            {
                paintManager.PaintSpace = paintSpace;
                MarkAsDirty();
            }

            if (isMeshObject && paintManager.PaintSpace == PaintSpace.World)
            {
                var projectionMethod = ProjectionMethod.Triplanar;
                var projectionMethodChanged = projectionMethodDrawer.Draw(projectionMethodProperty, "Projection Method", PaintManagerHelper.ProjectionMethodTooltip, ref projectionMethod);
                if (projectionMethodChanged)
                {
                    paintManager.Material.ProjectionMethod = projectionMethod;
                    MarkAsDirty();
                }

                var projectionDirection = ProjectionDirection.Normal;
                var projectionDirectionChanged = projectionDirectionDrawer.Draw(projectionDirectionProperty, "Projection Direction", PaintManagerHelper.ProjectionDirectionTooltip, ref projectionDirection);
                if (projectionDirectionChanged)
                {
                    paintManager.Material.ProjectionDirection = projectionDirection;
                    MarkAsDirty();
                }

                // Normal Cutout
                EditorGUI.BeginChangeCheck();
                EditorGUILayout.Slider(normalCutoutProperty, -1, 1, new GUIContent("Normal Cutout"));
                if (EditorGUI.EndChangeCheck())
                {
                    paintManager.Material.NormalCutout = normalCutoutProperty.floatValue;
                }
                ...
                // Angle
                EditorGUI.BeginChangeCheck();
                EditorGUILayout.Slider(projectionMaxAngleProperty, 0f, 180f, new GUIContent("Max Angle"));
                if (EditorGUI.EndChangeCheck())
                {
                    paintManager.Material.ProjectionMaxAngle = projectionMaxAngleProperty.floatValue;
                    MarkAsDirty();
                }
                
                var angleAttenuationMode = AngleAttenuationMode.Soft;
                var angleAttenuationModeChanged = angleAttenuationModeDrawer.Draw(projectionAngleAttenuationModeProperty, "Angle Attenuation Mode", PaintManagerHelper.ProjectionAngleAttenuationModeTooltip, ref angleAttenuationMode);
                if (angleAttenuationModeChanged)
                {
                    paintManager.Material.ProjectionAngleAttenuationMode = angleAttenuationMode;
                    MarkAsDirty();
                }
                ...
                // Brush Depth
                EditorGUI.BeginChangeCheck();
                EditorGUILayout.Slider(projectionBrushDepthProperty, 0f, 1f, new GUIContent("Brush Depth Size"));
                if (EditorGUI.EndChangeCheck())
                {
                    paintManager.Material.ProjectionBrushDepth = projectionBrushDepthProperty.floatValue;
                    MarkAsDirty();
                }

                EditorGUI.BeginChangeCheck();
                EditorGUILayout.Slider(projectionDepthFadeRangeProperty, 0f, 1f, new GUIContent("Brush Depth Fade Range"));
                if (EditorGUI.EndChangeCheck())
                {
                    paintManager.Material.ProjectionDepthFadeRange = projectionDepthFadeRangeProperty.floatValue;
                    MarkAsDirty();
                }
                ...
                // Edge
                EditorGUI.BeginChangeCheck();
                EditorGUILayout.Slider(projectionEdgeSoftnessProperty, 0f, 10f, new GUIContent("Edge Softness"));
                if (EditorGUI.EndChangeCheck())
                {
                    paintManager.Material.ProjectionEdgeSoftness = projectionEdgeSoftnessProperty.floatValue;
                    MarkAsDirty();
                }
            ...
            EditorGUI.BeginDisabledGroup(Application.isPlaying && (PaintController.Instance == null || PaintController.Instance.UseSharedSettings));
            var mode = PaintController.Instance != null && PaintController.Instance.UseSharedSettings ? PaintController.Instance.PaintMode : PaintMode.Default;
            if (Application.isPlaying && PaintController.Instance != null && PaintController.Instance.UseSharedSettings)
            {
                paintModeProperty.enumValueIndex = Array.IndexOf(Enum.GetValues(typeof(PaintMode)), PaintController.Instance.PaintMode);
            }
        ...
        private void DrawButtonsBlock()
        {
            ...
            EditorGUI.EndDisabledGroup();
            if (GUILayout.Button(new GUIContent("Reset", PaintManagerHelper.RedoTooltip), GUILayout.ExpandWidth(true)))
            {
